...
public class XDBCursor implements Iterator<DBObject>,
	Iterable<DBObject> {
	
	private static final int DEFAULT_FETCH_OFFSET = 0;
	private static final int DEFAULT_FETCH_LIMIT = 1000;
	
	private static Logger LOG = LogManager.getLogger(XDBCursor.class.getName());
    ...
    public XDBCursor(DBCollection coll, DBObject q, DBObject k){
    	_collection = coll;
     ...
     */
    public XDBCursor sort(DBObject orderBy){
        if (_it != null)
    ...
    public XDBCursor limit(Integer limit){
    	this._limit = limit;
    	return this;
    }
    
    public XDBCursor skip(Integer skip){
    	this._skip = skip;
    	return this;
    }
