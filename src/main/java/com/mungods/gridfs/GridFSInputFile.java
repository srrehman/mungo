...
public class GridFSInputFile extends GridFSFile {
    ...
    public void save( long chunkSize ) {
        if (_outputStream != null)
            throw new MungoException( "cannot mix OutputStream and regular save()" );
        ...
        if ( ! _savedChunks ) {
            try {
                ...
                throw new MungoException( "couldn't save chunks" , ioe );
            }
        }
    ...
    public int saveChunks( long chunkSize ) throws IOException {
        if (_outputStream != null)
            throw new MungoException( "cannot mix OutputStream and regular save()" );
        if ( _savedChunks )
            throw new MungoException( "chunks already saved!" );
        ...
        if ( chunkSize > 3.5 * 1000 * 1000 ) {
            throw new MungoException( "chunkSize must be less than 3.5MiB!" );
        }
