...
public abstract class GridFSFile implements DBObject {
    ...
    public void save(){
        if ( _fs == null )
            throw new MungoException( "need _fs" );
     ...
     */
    public void validate() throws MungoException {
        if ( _fs == null )
            throw new MungoException( "no _fs" );
        if ( _md5 == null )
            throw new MungoException( "no _md5 stored" );
        ...
        if ( res != null && res.containsField( "md5" ) ) {
            ...
            if ( m.equals( _md5 ) )
                return;
            throw new MungoException( "md5 differ.  mine [" + _md5 + "] theirs [" + m + "]" );
        }

        // no md5 from the server
        throw new MungoException( "no md5 returned from server: " + res );
